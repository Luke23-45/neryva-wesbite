...
| # | Work item | Routes | Status |
|---|---|---|---|
| 3.1 | Solutions | `/solutions` | DONE |
| 3.2 | Product marketing: Agent Studio | `/products/neryva-agent-studio` | DONE |
| 3.3 | Product marketing: AI Deployment | `/products/ai-deployment` | DONE |
...
|---|---|---|
| 2.x Deployment app | Shell mirrored from Studio (grouped nav Ship/Operate/Insights/Platform/Settings); all 19 views kit-adopted + tokenized via codemods; every dead button wired to real actions; detail pages gained restart/pause/promote/rollback feedback | ef4baf9 |
| 3.x Site pages | auth now calls the real REGISTER API (fake 3.5s timeout removed), honest links, autocomplete attrs; 404 ledger columns filled with real content + escape links; secret reveal states typed; solutions/product pages audited clean, canonicals fixed to real routes, motion preset adopted | 95bce84 |
| 4.1 Consistency | Zero dark-surface magic values remain across both apps (styles + views); all quoted ternaries tokenized with scoped theme params | ea47340 |
| 4.2-4.3 Hygiene + links | Dead code removed (orphan lab sections, deployment_ duplicate, .bak, scratch paths.tsx, untracked tsbuildinfo); footer/nav data purged of dead links and placeholder URLs; contact disclaimer no longer renders dead anchors | 8ceb45a |
| Close-out | Codemod damage repaired (nested imports, mangled gradients, stray destructors); `tsc -b` passes with ZERO errors | 6fc5e00 |
