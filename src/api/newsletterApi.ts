...
export const subscribe = async (data: SubscribeDTO): Promise<ApiResponse<null>> => {
  try {
    ...
    throw new Error(error.response?.data?.message || 'Could not subscribe. Please try again.');
  }
};
...
export const unsubscribe = async (data: UnsubscribeDTO): Promise<ApiResponse<null>> => {
  try {
    ...
    throw new Error(error.response?.data?.message || 'Could not unsubscribe. Please try again.');
  }
};
